...
def clipFeatures(lyr, aoi, featureFormat, zipFolderPath, scratchFolderPath, convertFeaturesDuringClip):
    ...
    try:
        ...
        if not convertFeaturesDuringClip:
            ...
            #Added by M.Norbury, Esri Canada, to handle KMZ files.  2014-03
            elif featureFormat[2].lower() in [".kmz"]:
                #Message "..using export to kmz.."
                gp.AddMessage("..using Layer to KML..")
                #The tool requires a feature layer as input.
                arcpy.MakeFeatureLayer_management(outputpath,"featureLayer")                
                arcpy.LayerToKML_conversion("featureLayer", outputinzip)
                #gp.LayerToKML_conversion("featureLayer", "E:\\TestKMZ\\test.kmz")
            else:
    ...
    try:
        ...
        for lyr in lyrs:
            ...
            if dataType in ["featurelayer", "rasterlayer"]:
                # M.Norbury, Esri Canada                            
                # Replace spaces and hyphens with underscores.  This will cause issues, due to the layer name no longer being found.               
                #lyr = lyr.replace(' ', '_').replace('_-_', '_')
                #Copy in the metadata xml file
                layerList = lyr.split(': ')
                category = layerList[0].replace(' ', '_').replace('_-_', '_')
                shortName = layerList[1].replace(' ', '_').replace('_-_', '_')
                metadataFile = metadataFolder + os.sep + 'Metadata' + os.sep + category + os.sep + shortName + '.xml'

                if os.path.exists(metadataFile):
                    shutil.copy(metadataFile, zipFolderPath)
...
if __name__ == '__main__':
    try:
        ...
        metadataFolder = r"\\agis-fs\geo-data\geoweb"
        ...
        #Added by M.Norbury, Esri Canada, to copy in the disclaimer file
        shutil.copy(metadataFolder + os.sep + "disclaimer.pdf", zipFolder)
        #Message "..copied in the disclaimer file.."
        gp.AddMessage("..copied in the disclaimer file..")
